...
既存の非空スキーマを取り込むために `spring.flyway.baseline-on-migrate=true` と `baseline-version=1` を設定しています。これは既存テーブル構造が `V1__create_member.sql` までの状態と一致している場合にだけ安全です。任意の中途半端なDBを修復する用途ではないため、構造が一致しない場合は無理にベースライン化せず、新しいDB/スキーマを用意してマイグレーションを最初から適用してください。
